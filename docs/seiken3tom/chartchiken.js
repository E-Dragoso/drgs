...
const index_pri_lv = 6;  // 主人公 Lv
const index_pri_tnl = 7; // 主人公 tNL
const index_fel1_lv = 8; // 仲間1 Lv
const index_fel1_tnl = 9;// 仲間1 tNL
const index_fel2_lv = 10;// 仲間2 Lv
const index_fel2_tnl = 11;// 仲間2 tNL
const index_lucre = 12;    // ルク(取得)
const index_sumlucre = 13; // ルク総額
const index_exp = 14;      // exp
const index_sumexppri = 15;// exp計主人公
const index_sumexpfel1 = 16;// exp計仲間１
const index_sumexpfel2 = 17;// exp計仲間２
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0; i < list_exp.length; i++) {
    ...
    // 主人公
    let pri_lv = tr.insertCell(); pri_lv.id = "pri_lv_" + i;
    let pri_tnl = tr.insertCell(); pri_tnl.id = "pri_tnl_" + i;
    // 仲間１
    let fel1_lv = tr.insertCell(); fel1_lv.id = "fel1_lv_" + i;
    let fel1_tnl = tr.insertCell(); fel1_tnl.id = "fel1_tnl_" + i;
    // 仲間２
    let fel2_lv = tr.insertCell(); fel2_lv.id = "fel2_lv_" + i;
    let fel2_tnl = tr.insertCell(); fel2_tnl.id = "fel2_tnl_" + i;
    // 取得ルク
    ...
    switch (list_type[i]) {
      // 雑魚
      case "雑魚":
        type.innerHTML = '<img src="rabi.webp">';
      ...
      // ルク行表示
      case "ルク":
        type.innerHTML = '<img src="lucre.webp">';
        tr.name = "lucre";
        break;
      // 買物行表示
      case "支出":
        type.innerHTML = '<img src="nikita.webp">';
      ...
      // ボス行表示
      case "ボス":
        type.innerHTML = '<img src="gova.webp">';
        bonus.firstChild.hidden = false;
        getexp.firstChild.hidden = false;
        break;
      case "強制":
        type.innerHTML = '<img src="rabi.webp">';
        type.classList.add("forced-row");
        bonus.firstChild.hidden = false;
        getexp.firstChild.hidden = false;
        break;
      // モールベアプリズン行表示
      case "育成":
        type.classList.add("training-row");
    ...
    // 1行目だけ手動で設定
    if (i == 0) {
      pri_lv.innerHTML = "Lv: 1";
      pri_tnl.innerHTML = "tNL: 30";
      fel1_lv.innerHTML = "Lv: 4";
      fel1_tnl.innerHTML = "tNL: 463";
      fel2_lv.innerHTML = "Lv: 6";
      fel2_tnl.innerHTML = "tNL: 1296";
      fel2_lv.classList.add("disabled-cell");
      fel2_tnl.classList.add("disabled-cell");
  ...
  for (i = 2; i < table.rows.length; i++) {
    for (j = 0; j < 3; j++) {
      ...
      while (exp >= Number(list_level[k])) {
        lv_val = "Lv: " + Number(k + 1);
        tnl_val = "tNL: " + (Number(list_level[k + 1]) - exp);
        k++;
      }
      
      // 分割したセルにそれぞれ代入
      let cell_lv = table.rows[i].cells[index_pri_lv + (j * 2)];
      let cell_tnl = table.rows[i].cells[index_pri_tnl + (j * 2)];
      
      cell_lv.innerHTML = lv_val;
      cell_tnl.innerHTML = tnl_val;
      
      // レベルアップ時の色変更（LvセルとtNLセルの両方に適用）
      let prev_cell_lv = table.rows[i - 1].cells[index_pri_lv + (j * 2)];
      if (cell_lv.textContent !== prev_cell_lv.textContent) {
        cell_lv.classList.add("levelup");
        cell_tnl.classList.add("levelup");
      } else {
        cell_lv.classList.remove("levelup");
        cell_tnl.classList.remove("levelup");
      }
      
      // ジャドまでは仲間2をグレー表示
      if (i <= pierrerow) {
        table.rows[i].cells[index_fel2_lv].classList.add("disabled-cell");
        table.rows[i].cells[index_fel2_tnl].classList.add("disabled-cell");
      } else {
        table.rows[i].cells[index_fel2_lv].classList.remove("disabled-cell");
        table.rows[i].cells[index_fel2_tnl].classList.remove("disabled-cell");
      }
    }
  }
}
