...
function characheck() {
  ...
  // 対象とする全キャラクターのクラス名リスト
  const charaClasses = ['Duran', 'Angela', 'Kevin', 'Charlotte', 'Hawkeye', 'Riesz'];

  // 1. まず全てのキャラクター関連アビリティ（チェックボックスと文字）を一括非表示にする
  charaClasses.forEach(className => {
    let elements = document.getElementsByClassName(className);
    for (let i = 0; i < elements.length; i++) {
      let el = elements[i];
      
      // 【ポイント】inputタグを包む親要素（labelなど）があれば親ごと消し、無ければ自分を消す
      if (el.parentNode && (el.parentNode.tagName === 'LABEL' || el.parentNode.classList.contains('dmg_group'))) {
        el.parentNode.hidden = true;
      } else {
        el.hidden = true;
        // テキストノードが横にある場合の対策として、テキストごと囲むクラスが無ければ非表示を徹底
        if (el.nextSibling && el.nextSibling.nodeType === 3) {
          // テキスト単体をJSで非表示にするため、一時的に親の表示も確認
  ...
  // セレクトボックス要素の一括非表示
  charaClasses.forEach(cls => {
    const classEl = document.getElementById(`${cls}Class`);
    const weaponEl = document.getElementById(`${cls}Weapon`);
    if (classEl) classEl.hidden = true;
    if (weaponEl) weaponEl.hidden = true;
  });

  // 2. 選択されたキャラクターに対応するアビリティ・選択ボックスのみを表示する
  if (charaClasses.includes(selectedchara)) {
    // セレクトボックスの表示
    const classEl = document.getElementById(`${selectedchara}Class`);
    const weaponEl = document.getElementById(`${selectedchara}Weapon`);
    if (classEl) classEl.hidden = false;
    if (weaponEl) weaponEl.hidden = false;

    // チェックボックスとテキスト（親要素）の表示
    let activeElements = document.getElementsByClassName(selectedchara);
    for (let i = 0; i < activeElements.length; i++) {
      let el = activeElements[i];
      el.style.display = ''; // input自体の非表示対策を解除
